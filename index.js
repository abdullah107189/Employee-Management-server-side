...
async function run() {
    try {
        ...
        app.post('/setUser', async (req, res) => {
            const user = req.body;
            const findEmail = await userCollection.findOne({ 'userInfo.email': user.userInfo.email })
            if (findEmail) {
                return res.status(409).send('Conflict: Image already exists in the collection');
            }
        ...
        app.get('/work-sheet/:email', async (req, res) => {
            ...
            const result = await workSheetCollection.find({ email }).sort({ date: -1 }).toArray()
            res.send(result)
        })
